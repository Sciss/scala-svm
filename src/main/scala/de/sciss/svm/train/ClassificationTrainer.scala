...
//    //    }
//
//    val modelSV = (0 until numClasses).flatMap { i =>
//      val nz  = nonZero(i)
//      val gi  = groups(i)
//      val ci  = gi.size
//      (0 until ci).collect {
...
//        for (k <- 0 until cj ) {
//          if(nonZero(j)(k)) {
//            modelSVCoef(i)(q) = f(p).alpha(ci+k)
//            q += 1
//          }
//        }
//        p += 1
...
//    }
//
//    new Model(numClasses = numClasses, param = param, supportVectors = modelSV, rho = f.map(_.rho)(breakOut)) {
